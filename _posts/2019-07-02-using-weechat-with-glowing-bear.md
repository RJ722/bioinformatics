...
{% newthought 'Last' %} month, I had a new addition to my toolbox - [Glowing
...
But, before we begin - the curious folks might ask, <i>"What had you been using
all this time?"</i>

Previously, I could directly ssh {% sidenote "ssh-id" "using mosh - mobile
shell, which supports intermittent connectivity, which is to say that it allows
me to retain the session if the network breaks in between." %} into my server,
and attach to a [`screen`][screen] session which ran weechat, using the command:

```
mosh USERNAME@SERVER_IP -- screen -D -RR weechat
```

<i> "This is pretty neat, why would you want to change it? </i>
...
server for listening to messages when AFK {% sidenote "" "**A**way **F**rom
**K**eyboard" %}. It was getting pretty unmanageable, so I decided to look for
alternatives. Luckily, one of my friends, [Armageddon][armageddon], pointed me
towards [ Glowing Bear ][gb] - which provides a nice web frontend to weechat.
...
<i> "Can't other people listen to my messages?" </i>
